...
from generate_chime7_data import choose_txt_normalization
...
def prepare_chime6(
    ...
    recording_set.to_file(output_dir / f"chime6-{mic}_recordings_{dset_part}.jsonl.gz")
...
def prepare_dipco(
    ...
    recording_set.to_file(output_dir / f"dipco-{mic}_recordings_{dset_part}.jsonl.gz")
...
def prepare_mixer6(
    ...
    if mic == "ihm":
        ...
        for sess in all_sessions:
            ...
            for idx, segment in enumerate(transcript):
                spk_id = segment["speaker"]
                channel = list(range(len(sess2audio[sess])))
                start = float(segment["start_time"])
                end = float(segment["end_time"])
                if ignore_shorter is not None and (end - start) < ignore_shorter:
                    print(
