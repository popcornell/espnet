...
def score_single(dasr_root, hyp_folder, output_folder, partition, diarization=True):
    ...
    for indx, scenario in enumerate(scenarios):
        hyp_json = os.path.join(hyp_folder, scenario + ".json")
        if bool(ignore_missing):
            warnings.warn(
        ...
        uem = os.path.join(dasr_root, scenario, "uem", partition, "all.uem")
        assert (
    ...
    sess_wise_df.to_csv(os.path.join(output_folder, "sessions_stats.csv"))
    spk_wise_df.to_csv(os.path.join(output_folder, "speakers_stats.csv"))
    scenario_wise_df.to_csv(os.path.join(output_folder, "scenarios_stats.csv"))
...
if __name__ == "__main__":
    ...
    """
    dasr_root = ""
    out_folder = "./scoring_participants"
    participants_preds = "/home/samco/Desktop/CHiME_7_preds/"
    track = "sub"
    import glob
    import os
    from pathlib import Path

    chime6_preds = glob.glob(os.path.join(participants_preds, "**/chime6.json"),
                             recursive=True)

    chime6_preds = [x for x in chime6_preds if track in [x.stem for x in
                                                         Path(x).relative_to(
                                                             participants_preds).parents]]
    for c in chime6_preds:
        c_dir = Path(c).parent
        sys_team = \
        [x.stem for x in Path(c).relative_to(participants_preds).parents][-2]
        sys_system = "sys1" if Path(c).parent.stem in ["eval", "dev"] else Path(
            c).parent.stem

        partition = \
        [x.stem for x in Path(c).relative_to(participants_preds).parents if
         x.stem in ["dev", "eval"]][0]

        macro_score = score_single(dasr_root, c_dir, partition, c_dir)
        print(f"TEAM: {sys_team}, SYS: {sys_team}, PART {partition}, SCORE: {macro_score} ")

        macro_score.insert(0, "track", track)
        macro_score.insert(0, "partition", partition)
        macro_score.insert(0, "sys_tag", sys_system)
        macro_score.insert(0, "team", sys_team)
