...
class DiarizationDataset(TorchDataset):
            ...
            if self.chunk_duration is None:
                ...
                while start < duration:
                    ...
                    if end - start >= self.min_chunk_duration:
                        # Filter: only include chunks that have at least one supervision (speaker)
                        # This avoids training on silence-only chunks which cause the model to always predict empty set
                        has_speakers = any(
                            sup.start < end and sup.start + sup.duration > start
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.chunk_duration is not None:
            ...
            # After truncation, supervisions are relative to the truncated cut (start at 0)
            # So we use 0 and chunk_duration for label generation
            label_start_time = 0.0
            label_end_time = chunk_duration
        else:
            # Full recording: use original times
        ...
        # After truncation, supervisions are relative to truncated cut (start at 0)
        labels = self._generate_frame_labels(cut, label_start_time, label_end_time)
...
def collate_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    ...
    # ESPnet3 Lightning expects batch = (ids, dict_of_model_kwargs); model expects waveform/waveform_lengths
    return (
        utt_ids,
        {
            "waveform": speech,
            "waveform_lengths": speech_lengths,
            "labels": labels,
...
class CollateFnWrapper:
    """Hydra-instantiable wrapper for collate_fn (takes no constructor args)."""
