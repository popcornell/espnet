...
def symlink_ami_manifests(
    source_dir: str,
    output_dir: str,
) -> None:
    """Symlink AMI IHM lhotse cut manifests so recipe finds train_cuts/dev_cuts/test_cuts.

    Expects source_dir to contain ami-ihm_cuts_train.jsonl.gz, ami-ihm_cuts_dev.jsonl.gz,
    ami-ihm_cuts_test.jsonl.gz. Creates output_dir with train_cuts.jsonl.gz, etc.
    """
    ...
    for link_name, target_name in mapping.items():
        ...
        if not src_file.exists():
            raise FileNotFoundError(
                f"AMI manifest not found: {src_file}. Expected lhotse cuts in {source_dir}."
            )
        if link_file.exists():
            link_file.unlink()
        link_file.symlink_to(src_file.resolve())
        logger.info("Symlinked %s -> %s", link_file, src_file)
    logger.info("AMI manifests ready in %s", out_path)
...
def create_dataset(
    ...
    """Symlink AMI manifests: use symlink_ami_manifests(source_dir, output_dir) for AMI.
