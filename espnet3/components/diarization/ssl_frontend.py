...
class SSLFrontend(nn.Module):
    ...
    def _infer_num_layers(self) -> int:
        ...
        # XEUS specific: use actual encoder output count (encode(..., use_final_output=False) returns list of this length)
        if self.is_xeus and hasattr(self.upstream, "encoder"):
            enc = self.upstream.encoder
            if hasattr(enc, "encoders"):
                return len(enc.encoders)
            if hasattr(enc, "layers"):
                return len(enc.layers)
            return 25  # fallback
    ...
    def _extract_features_xeus(
        ...
        # XEUS/ESPnetSSL encode returns dict with "encoder_output" (list of layer tensors)
        # and "encoder_output_lengths". Signature: encode(speech, speech_lengths, text=None, text_lengths=None, use_final_output=True)
        out = self.upstream.encode(
            waveform,
            lengths,
            use_final_output=False,  # Get all layer outputs, not just final
        )
        all_layer_outputs = out["encoder_output"]  # list of (batch, time, hidden_size)
        lengths = out["encoder_output_lengths"]
    ...
    def forward(
        ...
        if self.weight_sum is not None:
            ...
            # XEUS may return different layer count than inferred; adapt weight_sum on first forward
            n_layers = features.shape[-1]
            if n_layers != self.weight_sum.in_features:
                self.num_layers = n_layers
                self.weight_sum = nn.Linear(n_layers, 1, bias=False).to(features.device)
