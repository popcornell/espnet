...
class PowersetDiarizationModel(nn.Module):
    ...
    def compute_loss(
        ...
        """
        # Compute loss in fp32 for numerical stability (avoids NaNs from softmax/log in mixed precision)
        logits = logits.float()
        ...
        # Compute statistics (align lengths: model may output fewer frames than targets)
        with torch.no_grad():
            num_frames_logits = logits.shape[1]
            num_frames_targets = targets.shape[1]
            min_frames = min(num_frames_logits, num_frames_targets)
            logits_acc = logits[:, :min_frames, :]
            targets_acc = targets[:, :min_frames, :]
            lengths_acc = lengths.clamp(max=min_frames) if lengths is not None else None
            ...
            if best_permutations is not None and self.use_pit:
                ...
                targets_permuted = torch.zeros_like(targets_acc)
                for b in range(batch_size):
                    perm = best_permutations[b]
                    targets_permuted[b] = targets_acc[b, :, perm]
                target_powerset = self.powerset.to_powerset(targets_permuted)
            else:
                target_powerset = self.powerset.to_powerset(targets_acc)

            # Predicted powerset classes
            pred_powerset = torch.argmax(logits_acc, dim=-1)

            # Frame-level accuracy
            if lengths_acc is not None:
                from espnet2.legacy.nets.pytorch_backend.nets_utils import make_pad_mask
                # make_pad_mask returns (batch, max_len) with True for padding
                # We need to expand mask to match pred_powerset shape (batch, frames)
                pad_mask = make_pad_mask(lengths_acc, maxlen=pred_powerset.shape[1])
                mask = ~pad_mask  # True for valid frames
                matches = (pred_powerset == target_powerset) & mask
                num_valid_frames = mask.sum().float()
                accuracy = matches.sum().float() / num_valid_frames.clamp(min=1)
                
                # Debug: log prediction distribution occasionally
                import random
                if random.random() < 0.05:  # Log ~5% of batches for debugging
                    unique_preds, counts_preds = torch.unique(pred_powerset[mask], return_counts=True)
                    unique_targets, counts_targets = torch.unique(target_powerset[mask], return_counts=True)
                    pred_dist = dict(zip(unique_preds.cpu().tolist(), counts_preds.cpu().tolist()))
                    target_dist = dict(zip(unique_targets.cpu().tolist(), counts_targets.cpu().tolist()))
                    logging.info(f"[DEBUG] Predicted powerset distribution: {pred_dist}")
                    logging.info(f"[DEBUG] Target powerset distribution: {target_dist}")
                    logging.info(f"[DEBUG] Accuracy: {matches.sum().item()}/{num_valid_frames.item():.0f} = {accuracy.item():.4f}")
            else:
    ...
    def inference(
        ...
        # Debug: log statistics to diagnose low probability issue
        if torch.rand(1).item() < 0.05:  # Log 5% of calls for debugging
            powerset_probs = F.softmax(logits, dim=-1)
            # Check which powerset class has highest probability on average
            avg_powerset_probs = powerset_probs.mean(dim=(0, 1))  # Average over batch and time
            top_classes = torch.topk(avg_powerset_probs, k=3)
            logging.info(f"Top 3 powerset classes (avg prob): {dict(zip(top_classes.indices.cpu().tolist(), top_classes.values.cpu().tolist()))}")
            # Check if empty set (class 0) dominates
            empty_set_prob = avg_powerset_probs[0].item()
            logging.info(f"Empty set (class 0) average probability: {empty_set_prob:.4f}")
            # Log speaker activity statistics
            logging.info(f"Speaker activities: min={speaker_activities.min():.6f}, max={speaker_activities.max():.6f}, mean={speaker_activities.mean():.6f}")
            # Check how many frames have any speaker above 0.5
            frames_with_speakers = (speaker_activities.max(dim=-1)[0] > 0.5).sum().item()
            total_frames = speaker_activities.shape[0] * speaker_activities.shape[1]
            logging.info(f"Frames with any speaker prob > 0.5: {frames_with_speakers}/{total_frames} ({100*frames_with_speakers/total_frames:.1f}%)")
