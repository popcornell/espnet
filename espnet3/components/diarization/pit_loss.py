...
class PITLoss(nn.Module):
    ...
    def find_best_permutation_hungarian(
        ...
        # Replace NaN/Inf so Hungarian runs and gradients stay finite. Use max finite value
        # instead of a huge constant so we don't inject an artificial huge cost (which would
        # produce explosive gradients and make training harder).
        pairwise_losses = pairwise_losses.clone()
        if not torch.isfinite(pairwise_losses).all():
            n_invalid = (~torch.isfinite(pairwise_losses)).sum().item()
            n_nan = torch.isnan(pairwise_losses).sum().item()
            n_inf = torch.isinf(pairwise_losses).sum().item()
            logger.warning(
                "[SIGNIFICANT] PIT loss: cost matrix contained non-finite values (NaN=%d, Inf=%d, total invalid=%d). "
                "Replacing with max finite value for this batch. Consider using fp32 for the loss "
                "(e.g. cast logits/targets to .float() in compute_loss) or precision=32.",
                n_nan, n_inf, n_invalid,
            )
            finite = pairwise_losses[torch.isfinite(pairwise_losses)]
            fill = finite.max().item() if finite.numel() > 0 else 1e6
            pairwise_losses = torch.nan_to_num(
                pairwise_losses, nan=fill, posinf=fill, neginf=fill
            )
        ...
        for b in range(batch_size):
            cost_matrix = pairwise_losses[b].detach().cpu().float().numpy()
...
class PITLossWithPowersetEncoding(nn.Module):
    ...
    applies PIT (using BCE only to find the best permutation), then computes the actual
    loss with base_loss_fn (NLL/CE) on powerset classes. Only the powerset loss is backpropagated.
    ...
    def __init__(
        ...
        self.base_loss_fn = base_loss_fn  # Used for the actual loss: NLL/CE on powerset classes
        self.use_pit = use_pit

        if use_pit:
            # PIT finds best speaker permutation in multi-label space; BCE is only used
            # for that search. The backward pass uses base_loss_fn (NLL on powerset), not BCE.
    ...
    def forward(
        ...
        if not self.use_pit:
            ...
            # Apply log_softmax if using NLLLoss (it expects log-probabilities, not logits)
            if isinstance(self.base_loss_fn, nn.NLLLoss):
                logits_flat = F.log_softmax(logits_flat, dim=-1)
            ...
            batch_size, num_frames, num_classes = logits.shape
            num_speakers = self.powerset.num_classes
            ...
            for class_idx in range(num_classes):
                ...
                speaker_combination = self.powerset.mapping[class_idx]
            ...
            # Align targets to model output length (model may output fewer frames due to subsampling)
            target_frames = targets.shape[1]
            if target_frames > num_frames:
                targets = targets[:, :num_frames, :].contiguous()
                if lengths is not None:
                    lengths = torch.clamp(lengths, max=num_frames)
            elif target_frames < num_frames:
                logits_multilabel = logits_multilabel[:, :target_frames, :].contiguous()
                logits = logits[:, :target_frames, :].contiguous()
